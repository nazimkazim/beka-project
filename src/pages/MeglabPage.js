  ...
  const data = {
    ...
    functions: [
      ...
      },
      {
        moto: "Трекинг",
        icon: Reperage,
        desc: "На современной шахте безопасность рабочих должна иметь первостепенное значение. Благодаря отслеживанию Meglab предлагает эффективный способ узнать местонахождение каждого рабочего и транспортного средства в любое время. Отслеживание - это базовый модуль IMAGINE"
      },
      {
        moto: "Назначение ламп",
        icon: Lamp,
        desc: "Функция назначения ламп позволяет быстро назначать лампы рабочим в начале их смены. На станциях сотрудник сканирует свою карточку сотрудника и выделенную лампу. Теперь работник отслеживается с помощью модуля отслеживания."
      },
      {
        moto: "Вызов помощи",
        icon: Aide,
      ...
      },
      {
        ...
        icon: Reperage,
        desc: "Эта функция позволяет работнику получать доступ к разным зонам в соответствии с предоставленными полномочиями. Например, склад взрывчатых веществ заблокирован, пока тег не настроен, например, для предоставления доступа определенным людям."
      }
    ]
  };
  return (
